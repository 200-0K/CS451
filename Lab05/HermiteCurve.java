...
public class HermiteCurve extends JPanel {
    private BufferedImage image;
    private CurvePoints points;
    private int steps;

    HermiteCurve(int[] p0, int[] p1, int[] p0dash, int[] p1dash, int steps) {
    ...
    private int[] computePointsAtTime(float t) {
        ...
        // [t^3 + t^2 + t + 1] * Hermite matrix = Blending functions
        int[] newPoints = new int[2]; // new (x,y)
        for (int i = 0; i < newPoints.length; i++) {
            // (1x4 Matrix(Blending functions)) X (4x1 Matrix(Control Points))
            float c0 = (  2 * pow(t, 3) -  3 * pow(t, 2) + 1)  * points.p0[i];     // [2t^3 -3t^2 +1]  * [x or y]0
            float c1 = ( -2 * pow(t, 3) +  3 * pow(t, 2)    )  * points.p1[i];     // [-2t^3 +3t^2]    * [x or y]1
            float c2 = (      pow(t, 3) -  2 * pow(t, 2) + t)  * points.p0dash[i]; // [t^3 -2t^2 +t]   * [x or y]0'
            float c3 = (      pow(t, 3) -      pow(t, 2)    )  * points.p1dash[i]; // [t^3 -t^2]       * [x or y]1'
